  ...
  return (
    <div className="space-y-6">
      ...
      <section className="surface overflow-x-auto">
          ...
          <tbody className="divide-y divide-[#EEE7DD]">
            {(investorsQ.data ?? []).map((entry) => (
              <tr key={entry.id}>
                ...
                <td className="px-4 py-3 tabular-nums text-[#6F6659]">{entry.date.slice(0, 10)}</td>
        ...
        {!investorsQ.isLoading && !investorsQ.isError && !(investorsQ.data ?? []).length ? (
          <p className="px-4 py-10 text-center text-sm text-[#8A8072]">No investor entries yet.</p>
        ) : null}
      </section>
      ...
      {createOpen ? (
        ...
        >
          <div className="surface w-full max-w-lg p-5">
            <h3 className="text-lg font-semibold text-[#2A2A2A]">Add investor</h3>
            ...
            <div className="mt-4 flex items-center justify-end gap-2">
              ...
              >
                {createInvestor.isPending ? "Creating..." : "Create"}
              </button>
            </div>
          </div>
        </div>
